...
//
// MODIFICATIONS :
//   - Suppression du message "Plafond dépassé — uploads bloqués"
//   - Remplacement par un avertissement non-bloquant
//   - solde_restant peut être négatif (affiché en rouge)
...
// MODIFIÉ : ajout de warning_95 + suppression du label "Plafond dépassé — uploads bloqués"
const ALERT_CONFIG = {
  ...
  // NOUVEAU palier
  warning_95 : { color:"#ef4444", bg:"rgba(239,68,68,0.08)",   border:"rgba(239,68,68,0.3)",   icon:"🚨", label:"95% — Admin notifié"     },
  // MODIFIÉ : "exceeded" n'est plus un blocage, juste une alerte
  exceeded   : { color:"#ef4444", bg:"rgba(248,113,113,0.08)", border:"rgba(248,113,113,0.3)", icon:"🚨", label:"Seuil dépassé — Alerte"  },
};

const Spin = () => (
  <svg width="14" height="14" viewBox="0 0 20 20"
       style={{ animation:"spin-ring 0.8s linear infinite", flexShrink:0 }}>
...
export default function BudgetWidget({ userId, compact = false, onBlock }) {
  ...
  // MODIFIÉ : state error supprimé — les erreurs sont silencieuses
  ...
  const fetchBudget = useCallback(async () => {
    ...
    try {
      ...
      setbudget(data);
      // MODIFIÉ : onBlock toujours false — plus de blocage
      if (onBlock) onBlock(false);
    } catch (err) {
      // MODIFIÉ : erreur silencieuse — pas d'affichage à l'utilisateur
      // Le widget disparaît proprement si le backend est indisponible
      console.warn("[BudgetWidget] fetch error:", err.message);
    } finally {
      setLoading(false);
    }
  }, [userId, onBlock]);
  ...
  // MODIFIÉ : solde peut être négatif
  const isNegative = budget.solde_restant < 0;

  // ── Version compacte (mobile) ─────────────────────────────────────────────
  if (compact) return (
    <div style={{ padding:"12px 16px", borderRadius:12, background:cfg.bg,
                  border:`1px solid ${cfg.border}`, transition:"all 0.3s" }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", flexWrap:"wrap", gap:4 }}>
        <span style={{ fontSize:11, color:"#5a6e99" }}>
          {isNegative ? "Dépassement" : "Restant disponible"}
        </span>
        <span style={{ fontSize:11, fontWeight:600,
                       color: isNegative ? "#ef4444" : "#e8f0ff" }}>
          {isNegative
            ? `+${fmt(Math.abs(budget.solde_restant))} au-delà`
            : fmt(budget.solde_restant)
          }
        </span>
      </div>
      {/* MODIFIÉ : message avertissement non-bloquant si ≥ 95% */}
      {["warning_95", "exceeded"].includes(budget.alert_status) && (
        <div style={{ marginTop:10, padding:"8px 12px", borderRadius:8,
                      background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.25)",
                      fontSize:11, color:"#fca5a5", lineHeight:1.5 }}>
          🚨 Seuil atteint — l'administrateur a été notifié par email.
          <strong> L'upload reste autorisé.</strong>
        </div>
      )}
    </div>
  );
    ...
    <div className="surface-card" style={{ padding:"var(--card-pad)" }}>
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between",
        ...
        <div>
          ...
          <p style={{ fontFamily:"'Syne',sans-serif", fontSize:20,
      ...
      <div style={{ marginBottom:20 }}>
        <div style={{ display:"flex", justifyContent:"space-between",
          ...
          <span style={{ fontSize:12, color:"#5a6e99" }}>
            ...
            <em style={{ fontSize:10, color:"#3a4d72" }}>(notes de frais)</em>
          </span>
        ...
        <div style={{ height:10, borderRadius:5, background:"rgba(255,255,255,0.06)",
          ...
          <div style={{ position:"absolute", left:"80%", top:0, bottom:0,
                        width:1, background:"rgba(245,158,11,0.4)", zIndex:1 }}/>
          <div style={{ position:"absolute", left:"90%", top:0, bottom:0,
                        width:1, background:"rgba(249,115,22,0.4)", zIndex:1 }}/>
          <div style={{ position:"absolute", left:"95%", top:0, bottom:0,
                        width:1, background:"rgba(239,68,68,0.5)", zIndex:1 }}/>
          <div style={{
        ...
        <div style={{ display:"flex", justifyContent:"space-between", marginTop:5 }}>
          ...
          {/* NOUVEAU : marqueur 95% */}
          <span style={{ fontSize:9, color:"rgba(239,68,68,0.7)" }}>95%</span>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",
        ...
        <div style={{ padding:"14px 16px", borderRadius:12,
          ...
          <p style={{ fontSize:10, color:"#5a6e99", textTransform:"uppercase",
                      letterSpacing:"0.06em", marginBottom:6 }}>Plafond</p>
          ...
          <p style={{ fontSize:9, color:"#3a4d72", marginTop:4 }}>
            {budget.seuil_source === "user"
              ? "📌 Personnalisé"
              : budget.seuil_source === "role"
              ? "🏷 Par rôle"
              : "⚙ Défaut"}
          </p>
        </div>
        <div style={{ padding:"14px 16px", borderRadius:12,
          ...
          <p style={{ fontSize:10, color:"#5a6e99", textTransform:"uppercase",
                      letterSpacing:"0.06em", marginBottom:6 }}>Dépensé</p>
          <p style={{ fontFamily:"'Syne',sans-serif", fontSize:14,
        ...
        <div style={{
          ...
          background: isNegative ? "rgba(239,68,68,0.08)" : "rgba(16,185,129,0.07)",
          border:`1px solid ${isNegative ? "rgba(239,68,68,0.25)" : "rgba(16,185,129,0.2)"}`,
        }}>
          <p style={{ fontSize:10, color:"#5a6e99", textTransform:"uppercase",
                      letterSpacing:"0.06em", marginBottom:6 }}>
            {isNegative ? "Dépassement" : "Restant"}
          </p>
          <p style={{ fontFamily:"'Syne',sans-serif", fontSize:14,
                      fontWeight:700, color: isNegative ? "#ef4444" : "#10b981" }}>
            {isNegative
              ? `+${fmt(Math.abs(budget.solde_restant))}`
              : fmt(budget.solde_restant)
            }
          </p>
          <p style={{ fontSize:9, color:"#3a4d72", marginTop:4 }}>
            {isNegative ? "Au-delà du seuil" : "Disponible"}
          </p>
        </div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr",
        ...
        <div style={{ padding:"10px 14px", borderRadius:10,
          ...
          <p style={{ fontSize:13, fontWeight:600, color:"#5a6e99" }}>
            {fmt(budget.depenses_invoices)}
          </p>
        </div>
      </div>

      {/* MODIFIÉ : bannière d'alerte — sans "uploads bloqués" */}
      {budget.alert_status !== "ok" && (
        <div style={{
          padding:"12px 16px", borderRadius:10, background:cfg.bg,
          border:`1px solid ${cfg.border}`,
          display:"flex", alignItems:"flex-start", gap:10,
          animation:"fade-in 0.3s ease", marginBottom:14,
        }}>
          <span style={{ fontSize:16, flexShrink:0 }}>{cfg.icon}</span>
          <div>
            <p style={{ fontSize:13, fontWeight:600, color:cfg.color, marginBottom:2 }}>
              {/* MODIFIÉ : plus de message "uploads bloqués" */}
              {["warning_95", "exceeded"].includes(budget.alert_status)
                ? "Seuil critique atteint — Administrateur notifié par email"
                : `${budget.pct_utilise.toFixed(1)}% du plafond atteint`
              }
            </p>
            <p style={{ fontSize:12, color:cfg.color, opacity:0.75, fontWeight:300 }}>
              {["warning_95", "exceeded"].includes(budget.alert_status)
                ? "L'upload reste autorisé. Contactez votre administrateur si nécessaire."
                : "Vous approchez de votre limite mensuelle de notes de frais."
              }
            </p>
          </div>
        </div>
      ...
      <div style={{ display:"flex", justifyContent:"flex-end" }}>
        <button
          onClick={fetchBudget}
          style={{
            background:"none", border:"none", cursor:"pointer", color:"#5a6e99",
            fontSize:11, display:"flex", alignItems:"center", gap:6,
            padding:"6px 10px", borderRadius:8, transition:"color 0.2s", minHeight:36,
          }}
