...
// Envoie les données au backend FastAPI (/auth/login).
// Appelle onLogin(username) après un login réussi.
...
const LoginPage = ({ onLogin }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setSuccess(true);
        setLoading(false);
        setTimeout(() => onLogin(data.username), 900);
      } else {
        setLoading(false);
  ...
  // Style dynamique des champs selon l'état focus
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 16px", position: "relative", zIndex: 1 }}>
      ...
      {showResetModal && (
        <div
          style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.65)", backdropFilter: "blur(7px)", animation: "fade-in 0.25s ease" }}
          onClick={() => setShowResetModal(false)}
        >
          <div
            style={{ background: "linear-gradient(145deg,#0d1627,#0a1120)", border: "1px solid rgba(37,99,235,0.35)", borderRadius: 20, padding: "32px 36px", maxWidth: 360, width: "90%", boxShadow: "0 24px 60px rgba(0,0,0,0.65), inset 0 1px 0 rgba(255,255,255,0.06)", textAlign: "center", animation: "slide-up 0.35s cubic-bezier(0.22,1,0.36,1)" }}
      ...
      >
        ...
        {/* Affiche l'animation de succès ou le formulaire */}
        {success ? (
          ...
          <form onSubmit={handleLogin} noValidate>
            ...
            <div className="lfe4" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
              ...
              <a href="#" onClick={e => { e.preventDefault(); setShowResetModal(true); }} style={{ fontSize: 13, color: "#3b82f6", textDecoration: "none", fontWeight: 400, transition: "color 0.2s" }} onMouseEnter={e => e.target.style.color = "#93c5fd"} onMouseLeave={e => e.target.style.color = "#3b82f6"}>
                Forgot password?
              </a>
            </div>
